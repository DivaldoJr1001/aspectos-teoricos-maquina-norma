...
function codeValidator(codeLinesArray) {
    norma_global_objects_util_1.setInputCodeValid(validateCode(codeLinesArray));
}
exports.codeValidator = codeValidator;
function validateCode(codeLinesArray) {
    validatorStack = [];
    norma_global_objects_util_1.resetGoToDestinations();
    norma_global_objects_util_1.resetStartEndDestinations();
    ...
    for (var i = 0; i < codeLinesArray.length; i++) {
        ...
        if (lineParts.length === 1 && lineParts[0].charAt(0) === ':') {
            if (!norma_global_objects_util_1.getGoToDestination(lineParts[0].substr(1))) {
                norma_global_objects_util_1.addGoToDestinations(lineParts[0].substr(1), i);
            }
            else {
                return false;
            }
            ...
            switch (lineParts.length) {
                case 1:
                    switch (lineParts[0]) {
                        ...
                        case 'endif':
                            ...
                            norma_global_objects_util_1.addLoopDestinations(latestStack.line, i);
                            break;
                        case 'endwhile':
                            latestStack = validatorStack.pop();
                ...
                case 2:
                    if (lineParts[0] === 'inc' || lineParts[0] === 'dec' || lineParts[0] === 'set0') {
                        ...
                        if (!norma_global_objects_util_1.getGoToDestination(lineParts[1])) {
                            return false;
                        }
                    }
                    else {
